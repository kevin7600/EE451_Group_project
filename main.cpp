...
// first kernel, which performs Fourier transform on the image
__global__ void fourier_transform(double *in, double *out) {
	...
	for (int k = 0; k < h * w; k++) {
		...
		for (int t = 0; t < h * w; t++) {
			double angle = 2 * M_PI * t * k / num;
			realSum += in[t] * cos(angle);
		}
		out[k] = realSum;
	}
}
...
int main(int argc, char **argv) {
	...
	dim3 dimGrid((1024 / numThreads), (1024 / numThreads));

	// copying host onto the device
	cudaMemcpy(gpu_a, (double) a, sizeof(unsigned char) * h * w, cudaMemcpyHostToDevice);
	...
	cudaMemcpy(b, (unsigned char) gpu_b, sizeof(unsigned char) * h * w, cudaMemcpyDeviceToHost);
